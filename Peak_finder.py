...
def peaker (dir):
    ...
    # peak picking
    peaks = ng.peakpick.pick(data, (data.max() * config.Default_Threshhold), diag=False, cluster=True)

    # ppm conversion
    SW_ppm = SW_hz / B0_hz
    SO1_ppm = SO1_hz / B0_hz
    (xcorr_ppm, ycorr_ppm) = pw.fn_peak_corrector(peaks, data, B0_hz)
    peaks_ppm = pw.fn_peak_ppm(peaks, SW_ppm, len(data), xcorr_ppm, ycorr_ppm)
